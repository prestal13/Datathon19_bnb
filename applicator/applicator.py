...
class StaticParams():
    def __init__(self, term: int, contract_sum: float,
        ...
        assert self.car_category in [1, 2, 3, 4, 5]
...
class PickleLoader():
    ...
    def load(self, clf_path, est_path):
...
class Applicator():
    ...
    def generate_new_series(
        ...
        """ Makes New Series """
        return self.make_series(static_params, temporal_params, False, False)

    def complete_source_series(
            self, static_params : StaticParams, temporal_params : TemporalParams):
        """ Completes nans in source series"""
        return self.make_series(static_params, temporal_params, True, True)
    ...
    def update_output(self, output: OutputSeries, next_percent: float, static_params: StaticParams):
        contract_sum = static_params.contract_sum
        next_value = next_percent / 100 * contract_sum
        output.percent_series.append(next_percent)
    ...
    def make_series(self, static_params : StaticParams, temporal_params : TemporalParams,
        ...
        for id_period in range(1, static_params.term + 1):
            features = [static_params.term, static_params.contract_sum, static_params.idx_gender,
                    ...
                    static_params.ratio_10, static_params.ratio_20, static_params.ratio_30,
                    static_params.ratio_40, state.is_grace_constant]
            ...
            if real_state_update:
                self.update_state(state, real, static_params, id_period)
            else:
                self.update_state(state, estimate, static_params, id_period)
            if real_output_update:
                self.update_output(output, real, static_params)
            else:
                self.update_output(output, estimate, static_params)
